...
INFER_PROMPT = """Given a crop disease or pest name, return the most likely crop plant it affects.
Return ONLY valid JSON with one key: "crop_name" (string).
Examples: Early Blight → Tomato, Gray Leaf Spot → Maize, Late Blight → Potato,
Fall Armyworm → Maize, Leaf Rust → Wheat, Yellow Mosaic → Soybean.
If unknown, return the most common crop affected."""


def _infer_crop_from_disease(disease_name: str) -> str:
    """Ask the LLM to guess the crop from the disease name."""
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Disease/pest: {disease_name}. What crop does this affect?"},
            ],
            response_format={"type": "json_object"},
            temperature=0.1,
        )
        data = json.loads(response.choices[0].message.content)
        return data.get("crop_name", "Tomato")
    except Exception:
        return "Tomato"
...
def get_market_advice(crop_name: str, severity: int, recovery_days: int, disease_name: str = "") -> dict:
    price_data = _lookup_price(crop_name)

    # Fallback: infer crop from disease name when crop is Unknown or not in CSV
    inferred = False
    if price_data is None and disease_name and disease_name not in ("None", ""):
        inferred_crop = _infer_crop_from_disease(disease_name)
        price_data = _lookup_price(inferred_crop)
        if price_data:
            inferred = True
            crop_name = inferred_crop
